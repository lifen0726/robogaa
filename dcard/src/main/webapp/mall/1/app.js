  ...
  setup() {
    ...
    const fetchProductDetails = async (productId) => {
      const response = await axios.get(
        `http://localhost:25565/products/${productId}`
      );
    ...
    const fetchProducts = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:25565/products?${params}`
        );
    ...
    const deleteProduct = async (id) => {
      try {
        await axios.delete(`http://localhost:25565/products/${id}`);
        fetchProducts();
      } catch (error) {
        console.error(error);
      }
    ...
    const submitForm = async () => {
      ...
      try {
        if (isEditing.value) {
          await axios.put(
            `http://localhost:25565/products/${editingId.value}`,
            form.value
          );
        } else {
          await axios.post("http://localhost:25565/products", form.value);
        }
        fetchProducts();
        form.value = {
          productName: "",
